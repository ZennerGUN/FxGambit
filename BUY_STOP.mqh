...
//                  "OnHold: " + OnHold    );
              ...
              NewCandelPoped)
            {
               ...
               RefreshRates();
               LastCandleStart = Time[0];
               //// 07222025 Print("Envelopes_Slider BUY_STOP SET: " + PriceTargetLevel);
            }
#endif
            ...
            // START BUY_STOP
            RefreshRates();
            if((Ask < PriceTargetLevel)) //  So that you can BUY at this level, AFTER Ask becomes GREATER than PriceTargetLevel
              {
               ...
               ObjectSetString(ChartID(),"PriceLevelProtectionValue",OBJPROP_TEXT,StringFormat("%02d. ",(NumOfTrys))+ASKPrefix+DoubleToStr(PriceTargetLevel,Digits)+Separator+
                                                                                    ...
                                                                                    ToEntry+DoubleToStr((PriceTargetLevel-Ask)/Point/_TicksPerPIP,1)+MeasurePips);
            ...
            else  //   OPEN POS LEVEL - TRIGGERED --->>> Ask >= PriceTargetLevel **************************************************************************************************
              {
                  ...
                  if(ExecCommand == BUY_STOP && 
                        ...
                        EnableCrossSynch == false)
                        {
                           ...
                           RefreshMarketRefPoint();
                           // 07222025 Print("Flipping Sides: BUY_LIMIT");
                           return;
                        }
                  ...
                  // Is it ENVELOP situation BUY         -  EXIT if out of range
                     ...
                     if(!GetCurrentLossFromGlobalVAR(AcumulatedFloatingLoss))
                     {
                       ...
                       if(UseEnvelopeSlider && EnvelopeSliderActive ) 
                        ...
                        //  This will make so that it NO LONGER trys to open BUY_STOP order
                        LastExecCommand = ExecCommand;
               		   //// 07222025 Print("Current ExecCommand: " + EnumToString(LastExecCommand));
...
//#include <RemoteReset2.mqh>
               ...
               ObjectSetString(ChartID(),"PositionOutcomeValue",OBJPROP_TEXT,StringFormat("%02d. ",NumOfTrys)+ASKPrefix+DoubleToStr(PriceTargetLevel,Digits)+Separator+
                                                                              ...
                                                                              DoubleToStr((PriceTargetLevel - Ask)/Point/_TicksPerPIP,1)+MeasurePips+Slippage+Separator+
               ...
               if(StopAdjustSlippage || ProfitAdjustSlippage || PriceTargetAdjustSlippage)
               {
                  ...
                  if(ShowPartialCloseLine && (StopLossPips > 0))
                  {
                    ...
                    else
                    {
                     ...
                     // 07222025 Print("Partial Close BUY_STOP can't be implemented!!!");
                    }    
                  }
#endif
               ...
               if(!StopLossLevelLineSELECTED && !TTriggerActivated)  //  DON"T refresh here WHEN TTrigger Activated as REFRESH is done from the actual TTrailing section
               {
                  ObjectSetString(ChartID(),"StopLossValue",OBJPROP_TEXT,BIDPrefix+DoubleToStr(StopLossLevel,Digits)+Separator+
                                                                         DoubleToStr(Bid,Digits)+Separator+
										                                           ToStopLoss+DoubleToStr((Bid-StopLossLevel)/Point/_TicksPerPIP,1)+MeasurePips+Separator+
...
//                                       StopLossLevel);   
                  ...
                  //// 07222025 Print(BIDPrefix+DoubleToStr(StopLossLevel,Digits)+Separator+
                  //DoubleToStr(Bid,Digits)+Separator+ToStopLoss+
                  //DoubleToStr((Bid-StopLossLevel)/Point/_TicksPerPIP,1)+MeasurePips+Separator+" PROT: "+
...
//  STOP HIT BUY_STOP
               ...
               // 07222025 Print("<<< STOP HIT BUY_STOP >>>");
               ...
               //if( GlobalValSet(_GV_REFRESH_STOPS, NumOfStops) )
               //   //// 07222025 Print("GlobalVAL " + _GV_REFRESH_STOPS + " SET Successfully TO: " + NumOfStops);
               //else
               //   //// 07222025 Print("GlobalVAL " + _GV_REFRESH_STOPS + " CAN'T be SET TO: " + NumOfStops);                                     
               ...
               if(PartialCloseHit)
               {                
                     // 07222025 Print("<<<Stopped out by BUY_STOP PARTIAL CLOSE...>>>");
                     EmulateTakeProfitBUY_STOP();
...
//  SEND SIGNAL TO REMOTE EAs to REFRESH FUTURE LOT SIZE!!!
               ...
               //else if(!AutoFireAfterSL && ((MathMod(NumOfStops, AutoHoldPeriodSL) == 0) ))
     ...
     !OnHold))
   {
      ...
      // To be used if going with PENDING Position scenario...
            ...
            if(Bid < TakeProfitLevel) //  So that you can BUY at this level
              {
               ...
               if(!TakeProfittLevelLineSELECTED)
               {
                  ...
                  //ObjectSetString(ChartID(),"TrailingTriggerValue",OBJPROP_TEXT,BIDPrefix+DoubleToStr(TrailingTriggerLevel,Digits)+Separator+BIDPrefix+DoubleToStr(Bid,Digits)+Separator+ToProfit+DoubleToStr(NormalizeDouble(TrailingTriggerLevel-Bid,Digits)/Point/10,1)+MeasurePips);                  
              ...
              //  =============================
               ...
               // 07222025 Print("<<<  TOOK PROFIT - BUY_STOP " + DoubleToString(EA_INDEX_NUMBER,1));
#ifdef   _COMPENSATION_ENGINE_                   
...
//             BUY_LIMIT DOESN'T MIGRATE/MORPH INTO ANOTHER ORDER TYPE AFTER TAKE PROFIT (ONLY BUY_STOP & SELL_STOP MORPHS INTO A BUY_LIMIT & SELL_LIMITAFTER TAKE PROFIT
               LastExecCommand = ExecCommand;
         ...
         if(AutoRepeatAfterTP)
            {
               ...
               if(EnableCrossSynch)
                  {                  
                        ...
                        // 07222025 Print("<<< REINIT BUY_STOP >>>");
...
#endif                   
                     //  Remove Bot
                     // 07222025 Print("<<< GAME OVER - BUY_STOP >>>");
                     Sleep(SecPauseBeforeReRun * 1000);
                     //if(RemoveExpertAtEnd)
                        ExpertRemove();
//                  }
...
//#include <TransCompleteSET.mqh> 
               ...
               // 07222025 Print("<<< GAME OVER - BUY STOP>>>");
               Sleep(SecPauseBeforeReRun * 1000);
               if(RemoveExpertAtEnd)
                  ExpertRemove();
