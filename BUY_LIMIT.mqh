...
//                  "OnHold: " + OnHold    );
               ...
               NewCandelPoped)
            {
               ...
               RefreshRates();
               LastCandleStart = Time[0];
               // 07222025 Print("Envelopes_Slider BUY_LIMIT SET: " + DoubleToStr(PriceTargetLevel,2));
            }
#endif
            ...
            if((Ask > PriceTargetLevel)) //  So that you can BUY at this level, AFTER Ask becomes LESS than PriceTargetLevel
              {
               ...
               //  Ask:140.223(fixed) | Ask: 140.123(dynamic) | TO ENTRY TARGET: 10.3 pips
               ObjectSetString(ChartID(),"PriceLevelProtectionValue",OBJPROP_TEXT,StringFormat("%02d. ",(NumOfTrys))+ASKPrefix+DoubleToStr(PriceTargetLevel,Digits)+Separator+
                                                                                  DoubleToStr(Ask,Digits)+Separator+
                                                                                  ToEntry+DoubleToStr((Ask - PriceTargetLevel)/Point/_TicksPerPIP,1)+MeasurePips);
...
//               //// 07222025 Print("END OF WAITING TO OPEN POSITION...");
            ...
            else  //   OPEN POS LEVEL - TRIGGERED --->>> Ask <= PriceTargetLevel **************************************************************************************************
              {
                  if(ExecCommand == BUY_LIMIT && 
                     ...
                     EnableCrossSynch == false)
                     {
                        ...
                        RefreshMarketRefPoint();
                        // 07222025 Print("Flipping Sides: BUY_STOP");
                        return;
                     }
               ...
               {
                     StopLossLevel = Bid - (SLBufferPips * Point);
               ...
               {              
                  ...
                  if(isThereOpenPosition)
                  {
                     ...
                     //  This will make so that it NO LONGER trys to open BUY_LIMIT order
...
#endif                        
                     //  By RETURNING here the OPEN POSITION signal is CANCELED...
                     return;
                  }
                  ...
                  // Is it ENVELOP situation BUY         -  EXIT if out of range
                     ...
                     if(!GetCurrentLossFromGlobalVAR(AcumulatedFloatingLoss))
                     {
                        ...
                        // 07222025 Print("No AcumulatedFloatingLoss...");
...
//#include <RemoteReset2.mqh>
                        ...
                        //  Again -> Raise TransactionComplete = true so that it can respond to the REININT message that will be sent from the PROFIT TAKER!!!
                        //  No Need to have RemoteReset2.mqh -> duplicating functionality
               ...
               ObjectSetString(ChartID(),"ExecutePositionValue",OBJPROP_TEXT,DoubleToStr(Lots,Lot_Precision)+" LONG"+ATLevel+
               ...
               ObjectSetString(ChartID(),"PositionOutcomeValue",OBJPROP_TEXT,StringFormat("%02d. ",NumOfTrys)+ASKPrefix+DoubleToStr(PriceTargetLevel,Digits)+Separator+
                                                                             ...
                                                                             DoubleToStr((Ask - PriceTargetLevel)/Point/_TicksPerPIP,1)+MeasurePips+Slippage+Separator+
               ...
               if(StopAdjustSlippage || ProfitAdjustSlippage || PriceTargetAdjustSlippage)
               {
                  ...
                  if(ShowPartialCloseLine && (StopLossPips > 0))
                  {
                     ...
                     //if(PartialCloseMultiplierPercent / 100 * StopLossPips * Point >= BreakEvenPips)
                     {
                        DrawHorizontalLine(objPartialCloseLevelLineName,
                                           PartialCloseLevel,    
                                           PartialCloseLineStyle,
                    ...
                    else
                    {
                     ...
                     // 07222025 Print("Partial Close BUY_LIMIT can't be implemented!!!");
                    }
                  ...
                  {  
                     ...
                     // dBreakEvenLevel = NormalizeDouble(PriceTargetLevel + BreakEvenPips, Digits);     //OLD - Should be indipendent & outside PriceTargetAdjustSlippage scope...
	       	         //PriceTarget = PriceTargetLevel;
                     ...
                     // Outside of PriceTargetAdjustSlippage scope
                     // 
               ...
               if(SendEmailUpdates)
               {
                  ...
                  SendMail(EA_NAME_IDENTIFIER + " - " + OpenHeader,
                           ...
                           " LONG: "+DoubleToStr(Lots,2)+" Lots "+ATLevel+ DoubleToStr(CurrProfitLossPerPip,PL_PipPrecision)+" "+DepositCurrencyName+MeasurePerPip+"\n"+
                           ASKPrefix+DoubleToStr(PriceTargetLevel,Digits)+
                           Separator+ATLevel+ASKPrefix+DoubleToStr(ActualEntryPrice,Digits)+
         ...
         if(OrderOpened)
           {
            ...
            RefreshRates();
            if(Bid > StopLossLevel) //  So that you can SELL and CLOSE at this level
              {
               ...
               //  1.00 LONG |  00:00:10 sec.(dynamic)            |  Use LastStartTickTarget OR LastStartTickOpen matter of preference
               ...
               //  Bid:140.020(fixed) | Bid:140.210(dynamic) | TO STOP LOSS: 20.3 pips
               ...
               if(!StopLossLevelLineSELECTED && !TTriggerActivated)  //  DON"T refresh here WHEN TTrigger Activated as REFRESH is done from the actual TTrailing section
               {
                  ObjectSetString(ChartID(),"StopLossValue",OBJPROP_TEXT,BIDPrefix+DoubleToStr(StopLossLevel,Digits)+Separator+
                                                                         DoubleToStr(Bid,Digits)+Separator+ToStopLoss+
                                       									       DoubleToStr((Bid-StopLossLevel)/Point/_TicksPerPIP,1)+MeasurePips+Separator+
               ...
               if(TTriggerActivated)
               {
                  ...
                  //double NewStopLossLevel = (Bid - (TrailingTailPips * Point));
                  //double NewStopLossLevel = (Bid - MathAbs(TrailingTriggerLevel - TrailingTailLevel)/Point/_TicksPerPIP);
                  double NewStopLossLevel = (Bid - (TrailingStopPips * Point));
...
// ======================================================================================================
               ...
               // 07222025 Print("<<< STOP HIT BUY_LIMIT >>>");
               ...
               //Debug("Stop Loss Level Reached on Bid: "+DoubleToStr(Bid,Digits));
               OrderOpened=!OrderOpened;
               FirstTickOpen=!FirstTickOpen;
               FirstTickTarget=!FirstTickTarget;
               NumOfStops++;
               ...
               //ChangeColorForItem("PositionOutcomeHeaderValue");
               // STOPPED-OUT> Bid:140.020 | @Bid:140.025 | -0.05 pip slip | 08:18:15
               ObjectSetString(ChartID(),"PositionOutcomeValue",OBJPROP_TEXT,StringFormat("%02d. ",NumOfTrys)+BIDPrefix+DoubleToStr(StopLossLevel,Digits)+Separator+
                                                                             ATLevel+DoubleToStr(Bid,Digits)+Separator+
                                                                             DoubleToStr((Bid-StopLossLevel)/Point/_TicksPerPIP,1)+MeasurePips+Slippage+Separator+
                                                                             TimeToStr(TimeCurrent(),TargetTimeFormat));
               ...
               if(SendEmailUpdates)
               {
                  ...
                  SendMail(EA_NAME_IDENTIFIER + " - " + StopHeader,
                           ...
                           " Ticket# "+IntegerToString(CurOpenTicket)+
                           " LONG: "+DoubleToStr(Lots,2)+" Lots "+ATLevel+ DoubleToStr(CurrProfitLossPerPip,PL_PipPrecision)+" "+DepositCurrencyName+MeasurePerPip+"\n"+ 
                           BIDPrefix+DoubleToStr(StopLossLevel,Digits)+
                           Separator+ATLevel+DoubleToStr(ActualClosePrice,Digits)+
                           Separator+DoubleToStr((ActualClosePrice-StopLossLevel)/Point/_TicksPerPIP,1)+MeasurePips+Slippage+
               ...
               //ChangeColorForItem("PositionLocationValue");
               ObjectSetString(ChartID(),"ExecutePositionValue",OBJPROP_TEXT,"CLOSE "+DoubleToStr(Lots,Lot_Precision)+" LONG");
               //ChangeColorForItem("ExecutePositionValue");
               ...
               if(PartialCloseHit)
               {                     
                     // 07222025 Print("<<<Stopped out by BUY_LIMIT PARTIAL...>>>");
                     EmulateTakeProfitBUY_LIMIT();
               ...
               if(AutoLotIncrease)
                  Lots = CalcNewLotSize(AcumulatedFloatingLoss);                 
                  // 07222025 Print("New Calculated LOTS for Display purposes: " + DoubleToString(Lots));
#endif       
               ...
               if(!CheckEntryToExitDistance(PriceDir, ExecCommand, StopLossLevel, PriceTargetLevel, (Ask - Bid)))
                 {
                  ...
                  {
                     StopLossLevel = Bid - (SLBufferPips * Point);
                     //StopLossPips =  (PriceTargetLevel - StopLossLevel) / Point;
                     MoveHLine(objStopLossLevelLineName,StopLossLevel);
                     DrawArrow(objStopArrow,objStopArrow,StopLossLevel,StopArrow,StopArrowBackground,ANCHOR_TOP,StopArrowColor,StopArrowSize,StopArrowOffsetHor,StopArrowOffsetVer);
                     ObjectSetString(ChartID(),"StopLossValue",OBJPROP_TEXT,DoubleToStr(StopLossLevel,Digits)+Separator+
                                                                            DoubleToStr(((PriceTargetLevel - StopLossLevel)/Point/_TicksPerPIP),1)+MeasurePips);
     ...
     !OnHold))
   {
      ...
      // 07222025 Print("'>>> Attemting to OPEN EMERGENCY ORDER at BOTTOM of CHANNEL...");
...
if(ExecCommand==BUY_LIMIT)
            ...
            {
               // 07222025 Print(">>>  Couldn't OPEN Emergency order at BOTTOM of channel...");
            }
   }
   ...
   if(EnableCrossSynch && HitLiveMarket && FirstTimeRunAway)        // RunAway from Original Price Target
   {
      ...
      if(FirstTimeCrossEqualizeTP)
      {
            SendCrossEqualizeMessage(PriceTargetLevel + (TakeProfitPips * Point));
               ...
               else if(EnableCrossSynch && HitLiveMarket && FirstTimeRR)       //  11/30/2022 
               {
         ...
         if(OrderOpened && TakeProfitENABLED)
           {
            //  Handle TAKE_PROFIT
            RefreshRates();
            if(Bid < TakeProfitLevel ) //  So that you can BUY at this level
              {
               //  Bid:140.020(fixed) | Bid:140.210(dynamic) | TO TAKE-PROFIT: 20.3 pips
               if(!TakeProfittLevelLineSELECTED)
               {
                  ObjectSetString(ChartID(),"TakeProfitValue",OBJPROP_TEXT,BIDPrefix+DoubleToStr(TakeProfitLevel,Digits)+Separator+
                                                                           DoubleToStr(Bid,Digits)+Separator+
                                                                           ToProfit+DoubleToStr((TakeProfitLevel-Bid)/Point/_TicksPerPIP,1)+MeasurePips+Separator+
                                                                           DoubleToStr((TakeProfitLevel-PriceTargetLevel-ActualSlippage)/Point/_TicksPerPIP,1)+MeasurePips+Separator+
                                                                           //"<"+CalcPercentComplete(Bid,TakeProfitLevel,PriceTargetLevel)+">"+Separator+
            ...
            else   //  PROFIT HIT BUY_LIMIT
              {
               ...
               // 07222025 Print("<<<  TOOK PROFIT - BUY_LIMIT " + DoubleToString(EA_INDEX_NUMBER,1));
#ifdef   _COMPENSATION_ENGINE_                   
               ...
               //  Do something here...
               //Debug("Take Profit Level Reached on Bid: "+DoubleToStr(Bid,Digits));
               OrderOpened=!OrderOpened;
               FirstTickTarget=!FirstTickTarget;
               FirstTickOpen=!FirstTickOpen;
...
//  Get the latest value and increase it to reflect this last Profit taking and put it back in the Global Var so next time some other process can read it and increment it...               
               ...
               // Bid:140.020 | @Bid:140.025 | -0.05 pip slip | 08:18:15
               ObjectSetString(ChartID(),"PositionOutcomeValue",OBJPROP_TEXT,StringFormat("%02d. ",NumOfTrys)+BIDPrefix+DoubleToStr(TakeProfitLevel,Digits)+Separator+
                                                                             ATLevel+DoubleToStr(Bid,Digits)+Separator+
                                                                             DoubleToStr((Bid-TakeProfitLevel)/Point/_TicksPerPIP,1)+MeasurePips+Slippage+Separator+
                                                                             TimeToStr(TimeCurrent(),TargetTimeFormat));
               //ChangeColorForItem("PositionOutcomeValue");
               ...
               if(SendEmailUpdates)
               {
                  ...
                  SendMail(EA_NAME_IDENTIFIER + " - " + ProfitHeader,
                           ...
                           " Ticket# "+IntegerToString(CurOpenTicket)+
                           " LONG: "+DoubleToStr(Lots,2)+" Lots "+ATLevel+ DoubleToStr(CurrProfitLossPerPip,PL_PipPrecision)+" "+DepositCurrencyName+MeasurePerPip+"\n"+
                           BIDPrefix+DoubleToStr(TakeProfitLevel,Digits)+
                           Separator+ATLevel+DoubleToStr(ActualClosePrice,Digits)+
         ...
         if(AutoRepeatAfterTP)
            {
               ...
               if(EnableCrossSynch)
                  {                  
                        ...
                        // 07222025 Print("<<< REINIT BUY_LIMIT >>>");
...
//#endif 
//                    
//                    // 07222025 Print("Expert Adviser BUY_LIMIT has Successfully REINITIATED...");                             
//
//
//#ifdef   _COMPENSATION_ENGINE_  
//                 
...
#endif                   
                     //  Remove Bot
                     // 07222025 Print("<<< GAME OVER - BUY_LIMIT >>>");
                     Sleep(SecPauseBeforeReRun * 1000);
                     //if(RemoveExpertAtEnd)
                        ExpertRemove();
//                  }
...
//#include <TransCompleteSET.mqh> 
         ...
         if(DrawTTriggerLevel)        
           ...
           )
           {
            ...
            RefreshRates();
            if(Bid <= TrailingTriggerLevel) //  So that you can TRAIL from this level onwards
              {
               //  Bid:140.020(fixed) | Bid:140.210(dynamic) | TO TAKE-PROFIT: 20.3 pips
               if(!TTriggerLineSELECTED)
               {
                  ObjectSetString(ChartID(),"TrailingTriggerValue",OBJPROP_TEXT,BIDPrefix+DoubleToStr(TrailingTriggerLevel,Digits)+Separator+
                                                                                DoubleToStr(Bid,Digits)+Separator+
                                                                                ToTTarget+DoubleToStr((TrailingTriggerLevel-Bid)/Point/_TicksPerPIP,1)+MeasurePips+Separator+
                                                                                DoubleToStr((TrailingTriggerLevel-PriceTargetLevel-ActualSlippage)/Point/_TicksPerPIP,1)+MeasurePips+Separator+
                                                                                //"<"+CalcPercentComplete(Bid,TrailingTriggerLevel,PriceTargetLevel)+">"+Separator+                                                                                
            ...
            else   //  TRAILING TRIGGER HIT BUY_LIMIT
              {
                   ...
                   //  =============================
                   //  TTriger
                   //  First TIME Move SL to TrailingTailLevel Level!!!
...
//                        ObjectDelete(objTrailingTailLevelLineName);
      ...
      if(ShowPartialCloseLine && !PartialCloseHit)    
           if(OrderOpened)
           {
            ...
            RefreshRates();
            if(Bid <= PartialCloseLevel) //  So that you can PARTIAL CLOSE at this level 
              {
               ...
               {              
               if(Bid >= PriceTargetLevel) //  && TimeTicks > BeginingTicks)
               {
                  ...
                  // CLOSE position no mater how much in profit!
                     ...
                     if(uRet - InitialGoodUntilCancelTick >= GoodUntilCancelTimePeriodInMinutes * 1000 * 60)
                     {
                        ...
                        GetTicketClosePrice(CurOpenTicket, PartialCloseFinalLevel);
                     
                     //  01.15.2025 - Partial Close...               
                       DrawArrow(objPartialCloseFinalArrow,
                                 objPartialCloseFinalArrow,
                                 PartialCloseFinalLevel,
                                 PartialCloseFinalArrow,
                                 PartialCloseFinalArrowBackground,
                                 ANCHOR_BOTTOM,
                                 PartialCloseFinalArrowColor,
                                 PartialCloseFinalArrowSize,
                                 PartialCloseFinalArrowOffsetHor,
                                 PartialCloseFinalArrowOffsetVer
                                 );
                     	...
                     	//ChangeColorForItem("ExecutePositionValue");
                     	//  OPENED > 
                     	ObjectSetString(ChartID(),"PositionOutcomeHeaderValue",OBJPROP_TEXT,StagnantClose);
                     	//ChangeColorForItem("PositionOutcomeHeaderValue");
                     	//  OPENED > Ask:140.223 | @Ask:140.225 | -0.02 pip slip | 08:13:15
                     	ObjectSetString(ChartID(),"PositionOutcomeValue",OBJPROP_TEXT,StringFormat("%02d. ",NumOfTrys)+BIDPrefix+DoubleToStr(CurrBID,Digits)+Separator+
                                                                                   ATLevel+DoubleToStr(PartialCloseFinalLevel,Digits)+Separator+
                                                                                   DoubleToStr((PartialCloseFinalLevel - CurrBID)/Point/_TicksPerPIP,1)+MeasurePips+Slippage+Separator+
                                                                                   TimeToStr(TimeCurrent(),TargetTimeFormat));
               ...
               if(SendEmailUpdates)
               {
                  ...
                  SendMail(EA_NAME_IDENTIFIER + " - " + StagnantClose,
                           ...
                           " Ticket# "+IntegerToString(CurOpenTicket)+
                           " LONG: "+DoubleToStr(Lots,2)+" Lots "+ATLevel+ DoubleToStr(CurrProfitLossPerPip,PL_PipPrecision)+" "+DepositCurrencyName+MeasurePerPip+"\n"+
                           BIDPrefix+DoubleToStr(PartialCloseLevel,Digits)+
                           Separator+ATLevel+DoubleToStr(ActualClosePrice,Digits)+
                       ...
                       // No need to reset here as it is been reset to false at ReRun time!    
                       // FirstTimeGoodUntilCancel = false;  
                       
                       OverrideStatusUpdate = true;
                         
                        // 07222025 Print("<<<Order falling behind on schedule!  Stopped out by GOOD UNTIL CANCEL...>>>");
                        EmulateTakeProfitBUY_LIMIT();
                          
                       OverrideStatusUpdate = false;       
                        return;
...
//                     ObjectSetString(ChartID(),"PositionOutcomeHeaderValue",OBJPROP_TEXT,PartialClose);
                     ...
                     //ChangeColorForItem("PositionOutcomeHeaderValue");
                     //  OPENED > Ask:140.223 | @Ask:140.225 | -0.02 pip slip | 08:13:15
                     ObjectSetString(ChartID(),"PositionOutcomeValue",OBJPROP_TEXT,StringFormat("%02d. ",NumOfTrys)+BIDPrefix+DoubleToStr(PartialCloseLevel,Digits)+Separator+
                                                                                   ATLevel+DoubleToStr(Bid,Digits)+Separator+
                                                                                   DoubleToStr((PartialCloseFinalLevel - PartialCloseLevel)/Point/_TicksPerPIP,1)+MeasurePips+Slippage+Separator+
                                                                                   TimeToStr(TimeCurrent(),TargetTimeFormat));
                     ...
                     {  		               
   		               ...
   		               StopLossLevel = dBreakEvenLevel + PartialCloseBuffSL * Point;   //  Add 1 pip to dBreakEvenLevel for BUYs 
