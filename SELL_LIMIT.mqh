...
//                  "OnHold: " + OnHold    );
              ...
              NewCandelPoped)
            {
               ...
               RefreshRates();
               LastCandleStart = Time[0];
               //// 07222025 Print("Envelopes_Slider SELL_LIMIT SET: " + PriceTargetLevel);
            }
#endif
            ...
            // START SELL_LIMIT
            RefreshRates();
            if((Bid < PriceTargetLevel)) //  So that you can SELL at this level
              {
               ...
               ObjectSetString(ChartID(),"PriceLevelProtectionValue",OBJPROP_TEXT,StringFormat("%02d. ",(NumOfTrys))+BIDPrefix+DoubleToStr(PriceTargetLevel,Digits)+Separator+
                                                                                  ...
                                                                                  ToEntry+DoubleToStr((PriceTargetLevel - Bid)/Point/_TicksPerPIP,1)+MeasurePips);
            ...
            else  
              {
                  ...
                  if(ExecCommand == SELL_LIMIT && 
                     ...
                     EnableCrossSynch == false)
                     {
                           ...
                           RefreshMarketRefPoint();
                           // 07222025 Print("Flipping Sides: SELL_STOP");
                           return;
                     }
              ...
              if(!CheckEntryToExitDistance(PriceDir, ExecCommand, StopLossLevel, PriceTargetLevel, (Ask - Bid)))
               ...
               {           
                  ...
                  if(isThereOpenPosition)
                  {
                     ...
                     //  This will make so that it NO LONGER trys to open SELL_LIMIT order
...
//#include <RemoteReset2.mqh>
               ...
               ObjectSetString(ChartID(),"PositionOutcomeValue",OBJPROP_TEXT,StringFormat("%02d. ",NumOfTrys)+BIDPrefix+DoubleToStr(PriceTargetLevel,Digits)+Separator+
                                                                             ...
                                                                             DoubleToStr((PriceTargetLevel - Bid)/Point/_TicksPerPIP,1)+MeasurePips+Slippage+Separator+
               ...
               if(StopAdjustSlippage || ProfitAdjustSlippage || PriceTargetAdjustSlippage)
               {
                  ...
                  if(ShowPartialCloseLine && (StopLossPips > 0))
                  {
                    ...
                    else
                    {
                     ...
                     // 07222025 Print("Partial Close SELL_LIMIT can't be implemented!!!");
                    }  
                    }   
#endif
...
//               //// 07222025 Print("END OF WAITING TO CLOSE POSITION...");
            ...
            else     //  STOP HIT SELL_LIMIT
              {
               ...
               // 07222025 Print("<<< STOP HIT SELL_LIMIT >>>");
               ...
               {                
                     ...
                     // 07222025 Print("<<<Stopped out by SELL_LIMIT PARTIAL_CLOSE...>>>");
                     EmulateTakeProfitSELL_LIMIT();
         ...
         if(OrderOpened && TakeProfitENABLED)
           {
            ...
            if(Ask > TakeProfitLevel && TakeProfitENABLED) //  So that you can BUY at this level
              ...
              //  =============================
               ...
               // 07222025 Print("<<<  TOOK PROFIT - SELL_LIMIT " + DoubleToString(EA_INDEX_NUMBER,1));
         ...
         if(AutoRepeatAfterTP)
            {
               ...
               if(EnableCrossSynch)
                  {                  
                        ...
                        // 07222025 Print("<<< REINIT SELL_LIMIT >>>");
...
//
//                    
//                    // 07222025 Print("Expert Adviser SELL_LIMIT has Successfully REINITIATED...");                             
//
//
//#ifdef   _COMPENSATION_ENGINE_  
//                 
...
#endif                   
                     //  Remove Bot
                     // 07222025 Print("<<< GAME OVER - SELL_LIMIT >>>");
                     Sleep(SecPauseBeforeReRun * 1000);
                     //if(RemoveExpertAtEnd)
                        ExpertRemove();
//                  }
...
//#include <TransCompleteSET.mqh> 
               ...
               // 07222025 Print("<<< GAME OVER - SELL LIMIT>>>");
               Sleep(SecPauseBeforeReRun * 1000);
               if(RemoveExpertAtEnd)
                  ExpertRemove();
            ...
            else   //  TRAILING TRIGGER HIT BUY_LIMIT
              {
                  ...
                  if(TTriggerLineActive)
                  {
                     ...
                     ObjectSetString(ChartID(),"TrailingTriggerValue",OBJPROP_TEXT, ASKPrefix+DoubleToStr(TrailingTriggerLevel,Digits)+Separator+"ACTIVATED"); 
		               ObjectSetString(ChartID(), "TrailingTailValue", OBJPROP_TEXT,  ASKPrefix+DoubleToStr(TrailingTailLevel,Digits) + Separator + 
            ...
            if(Ask >= PartialCloseLevel) //  So that you can PARTIAL CLOSE at this level 
              {
               ...
               if(Ask <= PriceTargetLevel) //  && TimeTicks > BeginingTicks)
               {
                  ...
                  // CLOSE position no mater how much in profit!
                     ...
                     //ChangeColorForItem("PositionOutcomeHeaderValue");
                     //  OPENED > Ask:140.223 | @Ask:140.225 | -0.02 pip slip | 08:13:15
                     ObjectSetString(ChartID(),"PositionOutcomeValue",OBJPROP_TEXT,StringFormat("%02d. ",NumOfTrys)+ASKPrefix+DoubleToStr(CurrASK,Digits)+Separator+
                                                                                   ATLevel+DoubleToStr(Ask,Digits)+Separator+
                                                                                   DoubleToStr((PartialCloseFinalLevel - CurrASK)/Point/_TicksPerPIP,1)+MeasurePips+Slippage+Separator+
                                                                                   TimeToStr(TimeCurrent(),TargetTimeFormat));
            ...
            else   //  PARTIAL CLOSE HIT BUY_LIMIT
              {
               ...
               if(SendEmailUpdates)
               {
                  ...
                  SendMail(EA_NAME_IDENTIFIER + " - " + PartialClose,
                     ...
                     //ChangeColorForItem("PositionOutcomeHeaderValue");
                     //  OPENED > Ask:140.223 | @Ask:140.225 | -0.02 pip slip | 08:13:15
                     ObjectSetString(ChartID(),"PositionOutcomeValue",OBJPROP_TEXT,StringFormat("%02d. ",NumOfTrys)+ASKPrefix+DoubleToStr(PartialCloseLevel,Digits)+Separator+
                                                                                   ATLevel+DoubleToStr(Ask,Digits)+Separator+
